...
void Engine::run()
{
	...
	{	
		...
		//Tarkkailee tekeekö käyttäjä jotain
		while (SDL_PollEvent(&event))
		{
			switch (event.type)
			{
			//Nappia painetaan näppäimistöllä..
			case SDL_KEYDOWN:
				...
				//Meepo kutistuu pienemmäksi
				case SDLK_KP_2:
					s->setScale(s->getScale() + glm::vec2(-0.05f, -0.05f));
					break;

				//Meepo kääntyy myötäpäivään
				case SDLK_KP_6:
					s->setRotation(s->getRotation() + 3.5f);
					break;

				//Meepo kääntyy vastapäivään
				case SDLK_KP_4:
					s->setRotation(s->getRotation() - 3.5f);
					break;
